...
- **GPUs heterogéneas en `predict`/`feasibility` (límite conocido):** el predictor analítico y el análisis de viabilidad **asumen GPUs homogéneas** (n GPUs idénticas: `compute/n`); reciben un único tipo de GPU × `n_gpus`. El desbalance de hardware se modela y **valida** solo en el caso **GPU+CPU** (estrategia `heterogeneous` en `performance_model` + run de Verode, 0.12×). El **entrenamiento real sí admite GPUs distintas** (DDP da resultados correctos pero va al ritmo de la más lenta; model-parallel se equilibra con `--split-block`; y el `HeterogeneousDistributedSampler` reparte por `compute_weight`, generalizable a 2 GPUs distintas). Extender la **predicción** a conjuntos de GPUs heterogéneas (sumar throughputs en vez de ×n) queda como trabajo futuro — no se implementó porque no hay hardware con 2 GPUs distintas para validarlo, y la filosofía del proyecto es no predecir lo que no se puede contrastar con medidas reales.
