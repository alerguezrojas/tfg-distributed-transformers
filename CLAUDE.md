...
uv run paravit.py estimate --model vit_base_patch16_224 --gpu "Tesla T4" --strategy ddp --n-gpus 2 --precision amp
uv run paravit.py train --strategy {single|ddp|model-parallel|heterogeneous} --config ... [--dry-run]
uv run paravit.py benchmark --model vit_base_patch16_224 --batch-sizes 32,64 --epochs 30
uv run paravit.py eval --checkpoint checkpoints/local/checkpoint_epoch_009.pt --split test
uv run paravit.py dashboard
```

- Subcomandos: `train` (elige estrategia → lanza el script correcto, con `torchrun` para ddp/heterogéneo), `estimate` (antes `predict`; predictor **analítico** en terminal, sin GPU — tabla rich), `benchmark` (antes `feasibility`; **medición empírica** real en la máquina), `eval` (test split), `runs` (lista los entrenamientos de `logs/` con Best Val F1 / Test F1), `dashboard` (streamlit), `menu` (interactivo guiado — pregunta los parámetros, ideal para la defensa). **Renombrados el 24/06 (analítico/empírico)** y consolidados después: los builders y el paquete web también pasaron a `benchmark` (`build_benchmark_cmd`, `src/web/tabs/benchmark/`).
- Builders puros `build_train_cmd` / `build_benchmark_cmd` / `build_eval_cmd` + helper `_run_row` (testeados: `tests/unit/test_cli.py`). `paravit train --strategy model-parallel` también reenvía `--layers/--fn` (arreglado 25/06). `--dry-run` imprime el comando sin ejecutarlo (para copiarlo en Verode/Kaggle). Opciones de lista por coma (`--batch-sizes 32,64`, `--layers plot,confusion`).
- Los scripts de `scripts/` siguen funcionando por separado; `paravit` solo los unifica. Dependencia nueva: `typer`. Rama: `feature/unified-cli`.
...
`scripts/benchmark.py` (antes `check_feasibility.py`) — **medición empírica** previa al entrenamiento. Usa datos sintéticos (sin tocar el dataset) para medir throughput real y estimar tiempos. Comando: `paravit benchmark`.
...
**Motor analítico de predicción** (`src/performance_model.py`): **único motor** de predicción del proyecto. Predice tiempo/speedup/memoria/cuello/coste/**energía** **y calidad (F1)** de **cualquier** (estrategia, modelo, GPU, nº GPUs, dataset, batch, precisión) **sin benchmark** (fórmula maestra `T(n,π)`, estimación de `r_c/r_io/π` por specs, modelo de memoria/OOM). **Energía = potencia efectiva × tiempo** (29/06/26): `GpuSpec.train_power_w` calibrada de los logs `--fn energy` (T4 64W, V100 103W, RTX 3060 Ti 180W; resto = 0.65×TDP, `power_calibrated=False`), potencia independiente de precisión (AMP ahorra por *tiempo*), eval=0.9×train, `power_working_gpus` (single/heterogéneo=1, ddp/MP=n), MP×0.83 (idle de tubería); `predict()` también estima el tiempo de **eval** → total/epoch. Calibrado contra los datos reales de Kaggle (<10% en compute-bound). Expuesto en **Estimate → Predict** (tiempo + memoria + **energía** + coste + calidad + puente `rc_measured`). Derivación en `docs/performance_model.md`.
...
| **Estimate/Benchmark** (antes "Feasibility"→"Performance") | 3 pestañas, en orden: **Estimate** (antes "Predict"; predictor **analítico**: formulario → headline + **tablas con las fórmulas** de tiempo `max(compute,I/O)+sync`, memoria `weights+grad+Adam+activations+overhead` y **energía `potencia × tiempo`** con los valores puestos + calidad + escalado 1→8 GPU + coste nube; mismo motor que `paravit estimate`) · **Benchmark** (antes "Report"; **medición empírica** generada en terminal con `paravit benchmark`: hardware, throughput, tiempo, escalado distribuido, coste, estudio de convergencia) · **Estimate vs Benchmark vs Run** (antes "Benchmark vs Run"; comparación a **3 fuentes** — analítico / benchmark / run real — con **gráficas de barras agrupadas por métrica** (tiempo, energía, F1) en rojo/verde/azul, selector de fuente por gráfica, scorecard y scatter de calibración; el benchmark solo modela single+DDP, así que para MP/heterogéneo se ocultan sus barras y manda el analítico) |
...
- [x] **`paravit predict` enriquecido (rama):** el CLI ya no da solo números finales — muestra **el trabajo** como la tabla de fórmulas de la web: headline + **fórmula de tiempo** `max(compute, I/O) + sync` con cada término + **fórmula de memoria** (weights+grad+Adam + activations + overhead = total vs GPU, fits/OOM, max batch) + calidad + **tabla de escalado 1→8 GPUs** + **coste** (5 proveedores) + **supuestos** (r_c/r_io/params/MFU). Expuestos `t_compute_s/t_io_s/t_sync_s/batch_per_gpu` en `Prediction` (defaults, compatibles). Help de `dashboard` actualizado (Predict pasó al terminal y luego volvió también a la web).
- [x] **Predict de vuelta en la web (rama):** Feasibility pasa a **3 tabs: Predict · Compare vs runs · Report**. `src/web/tabs/feasibility/predict.py` (`_analytic_predictor`) replica el `paravit predict` enriquecido — formulario (modelo/GPU/estrategia/n_gpus/precisión/disco/dataset/batch/epochs) → headline + tablas de fórmula (tiempo + memoria) + calidad con curva + escalado 1→8 GPU (gráfica, solo distribuido) + coste nube + supuestos + expander de calibración con throughput medido. Solo calcula fórmulas, no entrena → respeta "la web mira, el terminal ejecuta".
...
- [x] **Suite en 367 tests.** README reescrito (sin emojis, requisitos previos + instalación + qué funciona sin GPU/dataset + entrenar de verdad; tutor/cotutor correctos) y luego adelgazado al mínimo (de qué va + requisitos + el menú). Diagrama de clases regenerado (paquete `feasibility/` con `predict.py`). Licencia **MIT** añadida (repo público). *(Cambiada el 26/06/2026 a **GPLv3** —copyleft— para el código; la memoria del TFG va bajo **CC BY-NC-SA 4.0**.)*
...
#### Auditoría estimate-vs-benchmark + fix de la proyección DDP plana (25/06/26)
- **División de tareas (para la defensa):** `estimate` (analítico, `performance_model`) cubre **las 4 estrategias** (single/DDP/model-parallel/heterogéneo) con la fórmula maestra. `benchmark` (empírico, `src/benchmark/`) **mide solo 1 GPU** (throughput/VRAM/energía reales) y **proyecta el escalado de DATOS (DDP)** con el `DDPOptimizer`; **model-parallel y heterogéneo NO los modela el benchmark** — los cubre el analítico + los runs reales (y la web los empareja en *Benchmark vs Run* con el fallback analítico). Auditoría multi-agente confirmó que es coherente; las limitaciones (GPUs distintas GPU+GPU, caché de RAM en el I/O, MFU calibrada en vit_base) ya están documentadas como tales.
- [x] **Fix: eliminada la proyección DDP "plana" del 0.85 (25/06/26, rama `fix/remove-flat-ddp-projection`):** `TimeEstimator` aún calculaba `ddp_total = tiempo/(n·0.85)` y lo escribía al CSV (`est_ddp_2gpu_h_*`), y la web lo mostraba en *Benchmark vs Run* ("DDP ×2 GPU total (est.)"), **contradiciendo** el `DDPOptimizer` preciso (compute/IO/sync). La auditoría del 24/06 quitó el número del informe de texto pero **no** estas columnas. Ahora el `DDPOptimizer` (bloque `#ddp`) es la **única fuente** del escalado DDP. Quitado de `time_estimator.py` + `report_formatter.py` (columnas CSV) + `benchmark_comparison.py` (fila web); 2 tests obsoletos → 1 test que fija la ausencia. **379 tests.** (Los CSV antiguos conservan las columnas pero ya no se leen → inofensivo.)

#### Auditoría profunda de todo el proyecto + pasada de limpieza (25/06/26, rama `fix/audit-cleanup-pass`)
Auditoría multi-agente de TODO el proyecto (núcleo, decoradores, estimador, benchmark, web, CLI, tests/docs) + **prueba empírica del estimador** sobre los 52 runs reales + **cruce de los datos** con lo documentado, con verificación adversarial de cada hallazgo. **Veredicto: el núcleo es correcto y los datos cuadran** (cada número de las tablas de CLAUDE.md coincide con los CSV/logs; el estimador acierta en régimen compute 1-6% y deriva solo en I/O como ya se documenta; OOM exacto). Se arreglaron los bugs reales encontrados (todos verificados):
- [x] **CLI: `train --strategy model-parallel` perdía `--layers/--fn`** (`cli.py`) → un MP lanzado desde el CLI/menú no generaba per-class/confusión/batch/energía. Reenviado + test actualizado. *(Los runs de Kaggle estaban bien: lanzados con el script directo.)*
- [x] **Reproducibilidad en DDP y heterogéneo** → `train_ddp.py`/`train_heterogeneous_ddp.py` siembran con offset por rank (`set_seed(seed+rank)` + generator/worker_init en el DataLoader); `set_seed` activa determinismo cuDNN.
- [x] **Heterogéneo descartaba `pos_weight`** → `HeterogeneousDDPTrainer._criterion_sum` hereda el `pos_weight` del criterion configurado (antes BCE sin pesos en train, distinto de val). *(Los runs hetero usaron BCE plano → válidos.)*
- [x] **Benchmark `--device N` medía la GPU 0** (potencia/VRAM) → usa el índice del dispositivo seleccionado.
...
- [x] Menores: guard de loader vacío (evita cuelgue DDP), `best_f1=-1.0` (siempre guarda baseline), métricas DDP solo en rank 0, radar de Compare sin fondo blanco (modo oscuro), imports muertos en `home.py`, `paravit benchmark --config <hetero>` ya no casca (KeyError). +2 tests (`retry_on_cuda_oom`).
...
#### Energía analítica + comparación a 3 fuentes "Estimate vs Benchmark vs Run" (29/06/26, rama `feature/analytic-energy-3way` → develop)
A petición del usuario: que el **estimador analítico** prediga también **energía** sin relanzar entrenamientos (usando los runs ya hechos), y que la pestaña "Benchmark vs Run" compare **tres fuentes** (analítico / benchmark / run) por métrica. Núcleo (vit_base, compute-bound) validado al **3-15%** en tiempo y energía en las 3 fuentes; **411 tests** (+29). Dos rondas adversariales multi-agente + auditoría empírica sobre los 52 runs.
- [x] **Energía analítica en `performance_model.py`:** misma física que el benchmark/run (`E = potencia × tiempo`), solo faltaba la potencia. `GpuSpec` gana `train_power_w`/`power_calibrated`, **calibrados de los logs `--fn energy`** (T4 64W, V100 103W, RTX 3060 Ti 180W; resto = `0.65×TDP` vía `_tdp_fallback_power`, marcado no calibrado). `estimate_power(gpu,phase)` (eval=`EVAL_POWER_FRACTION`=0.9×train), `power_working_gpus` (single/heterogéneo=1 — el 2º worker hetero es CPU; ddp/MP=n; MP×`MODEL_PARALLEL_POWER_FRACTION`=0.83 por el idle de tubería), `estimate_eval_time_s` (forward-only=3×rc, reparte solo en DDP). `predict()` gana `val_size`, estima tiempo de **eval → total/epoch** y rellena campos de energía en `Prediction` (`avg_power_w`, `power_total_w`, `energy_train/eval/per_epoch/total_wh`, `time_per_epoch_eval/total_s`, `power_calibrated`). Potencia **independiente de precisión** (AMP ahorra por tiempo, validado: T4 64W fp32 ≈ 63W amp). Guard de estrategia desconocida → `None`.
- [x] **`benchmark_comparison.py` a 3 vías:** `ComparisonRow` gana `analytic`/`analytic_error_pct`; `build_comparison` llama a `predict()` para la columna analítica, deriva la **energía de train real** del log (`energy_train_j/3600`) y añade filas **"Energy total / epoch"** y **"Best Val F1"**. **Recalcula el benchmark al tamaño del run** desde su `s/batch` medido (independiente del tamaño) → un run y un informe de distinto N siguen siendo comparables; corrige el benchmark fp32-1GPU a la config del run (tiempo ÷ precisión×DDP-speedup; **energía ÷ precisión solo** — DDP conserva la energía total cuando es compute-bound). `to_dataframe` con columnas Analytic/Benchmark/Real + Δ.
- [x] **Pestaña renombrada `Benchmark vs Run` → `Estimate vs Benchmark vs Run`:** `_run_record(r,m,fdf)` (puro, testeable) arma el registro 3-vías que pinta la web; **3 gráficas de barras agrupadas** (tiempo, energía, F1) en **rojo/verde/azul** con **selector de fuente por gráfica** (Both/Analytic/Benchmark, Real siempre); scorecard (error A y B vs real) + scatter de calibración. F1 con **3 decimales**.
- [x] **Energía también en Estimate (web) y `paravit estimate` (CLI):** tabla "Energy / epoch = power × time" + energía en el titular; `time_estimator.py` alinea la fracción de eval del benchmark (0.4→`EVAL_POWER_FRACTION`).
- [x] **Pasada de corrección profunda (auditoría sobre los 52 runs + 2 revisiones adversariales):** (1) **tamaño de dataset** del run, no del benchmark — y si se desconoce (run sin línea de config + benchmark sin `#sizes`) **no se inventa** (solo barra Real); (2) `_select_benchmark` elige el informe que **contiene el batch** del run; (3) mapa `ddp_hetero`→`heterogeneous` (antes el analítico del hetero reventaba); (4) **el benchmark no modela MP/heterogéneo** (mide 1 GPU sin I/O ni la CPU lenta → daba 0.12 min vs 4.22 real) → se **ocultan sus barras** de tiempo/energía para esas estrategias y manda el analítico; (5) `KeyError 'batch_size'` y `ValueError` de estrategia guardados; (6) captions honestos (energía DDP conservada solo en compute-bound; analítico alto en modelos ligeros/I-O-bound y heterogéneo por GPU infrautilizada; F1 analítico/benchmark = mismo prior).
- **Limitaciones documentadas (no bugs):** analítico **alto** cuando la GPU se infrautiliza (vit_tiny consume ~47W no ~64W + I/O cacheado; heterogéneo idlea ~45W esperando a la CPU); benchmark sintético **subestima** I/O-bound y no modela MP/hetero; logs DDP pre-fix (24/06) midieron 1 GPU (barra Real ~½). Pendiente: nota en `docs/performance_model.md` (energía) y merge a `main` tras validación en navegador.
